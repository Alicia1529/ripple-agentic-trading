...
`"strategy": "earnings_drift_v2"` in an Account Lane configuration. The
...
- `facts`: for every event candidate and held symbol, `close`, `atr20_pct`,
  ...
  `eps_surprise_pct`, `rev_actual`, `rev_consensus`,
...
Rank passing candidates by `eps_surprise_pct` descending, then
`rev_surprise_pct` descending, then symbol ascending. This version deliberately
uses only the named, source-verifiable surprise percentages; it does not require
an undefined standardized estimate-distribution metric. Research only the top
three in rank order. For each, answer exactly these questions with cited
evidence:
