...
3. **单一路径。** v1 只保留一个交易 broker、一个账户、一个策略、一个 agent 和一个调度任务。
...
| 决策层 | 3 analyst + PM LLM | 版本化确定性策略；AI 主要写报告 | 长期 v1 采用单 agent + deterministic baseline；multi-agent 只离线运行 |
...
| 范围 | 相对聚焦，但 4 次 LLM 调用链和多类数据源仍增加维护面 | 包含多券商、多策略账本、税务、dashboard，预估 120–240 小时 | 两者都要缩减；v1 只做单 agent、单策略、单 broker |
...
- 保留 Claude 的三 analyst + PM，但只先跑 **1 个 analyst + 1 个 deterministic baseline**。
...
- agent 与 baseline 同时产生 intended orders，但不下单。
...
- 不只看 P&L；分别评估 analyst direction、confidence calibration、PM 聚合增益、换手、滑点敏感性和风控拦截率。
...
- Schwab live、多策略虚拟账本和盘中高频率留到后续独立评审；它们扩大的是系统复杂度，而不只是资本风险。
...
一句话决策：**选 Claude 的“为什么和研究对象”，选 Codex 的“怎样证明它没有乱来，以及怎样慢慢给它权力”。**
