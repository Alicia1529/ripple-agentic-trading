...
- **Decision and execution are separate sessions**: the Decision Routine is forbidden from calling broker write tools; the current hosted Account A session exposes them, so this is a prompt-enforced MVP rule rather than a platform capability boundary. The next-morning Execution Routine may call them but must not redo investment reasoning.
