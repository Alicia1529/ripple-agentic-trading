...
This proves that Alicia's Codex connection is authenticated and can reach account-scoped reads. It does **not** provide a credential to the plain Python runner: at this historical point, the runner still needed its own bootstrap, token-storage, and refresh lifecycle. The Python SDK's restart gap is closed locally by Ripple's adapter, and the later runner-owned proof closed the local cross-process refresh gap; see `mcp-python-oauth-client.md`. A second account binding and production deployment remain unproven.
