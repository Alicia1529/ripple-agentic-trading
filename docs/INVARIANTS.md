...
5. **Decision and fill times stay honest.** A Day T signal never receives a Day T close fill. Live evaluation, Shadow Fills, and later replay use the documented prior-evening Decision and next-trading-day Execution semantics, resolved against the checked-in trading calendar rather than weekday arithmetic. A scheduled run outside a trading session produces no artifact.
...
9. **Mode selection is deterministic.** The catalog permits at most one live configuration, selects all shadow configurations in account-ID order, and excludes dry-run configurations from every scheduled cohort.
...
12. **Shadow evidence stays explicit.** A Shadow Fill requires deterministic risk permission and a marketable T+1 quote, records zero-fee/zero-slippage assumptions, and is never represented as a broker fill.
