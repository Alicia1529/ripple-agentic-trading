...
Account A's MVP strategy is the plain-language policy in `strategies/growth_momentum_v1.md`. The Decision LLM gathers the required facts for the full configured universe, applies the fixed trend/momentum rules, reviews current holdings, and may produce `NO_TRADE`, one new 10% BUY, one full discretionary SELL, or both. The existing generic `publish-decision` command validates and freezes the resulting OrderPlan; the existing execution risk code revalidates it before any broker action.

There is deliberately no strategy-specific Python engine, policy JSON, data-provider subsystem, or plugin framework. This keeps the small-account MVP inspectable and allows the same strategy prompt to cover later cycles after a position exists. The trade-off is explicit: code does not independently prove that the LLM screened every symbol or that its qualitative HOLD/SELL evidence is economically correct. Missing required data still produces no trade, target/order schemas remain strict, and deterministic Execution Risk Exits remain independent.
