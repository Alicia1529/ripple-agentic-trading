...
```
4:00 PM ET   Market close (Monday–Thursday decisions use that day's close;
             Sunday decisions use Friday's completed close plus weekend facts)
...
Account A's first runnable strategy is the deliberately narrow fixed policy `growth_momentum_v1` (`mvp1`). It accepts a credential-free fact document containing SPY close/SMA50 evidence, QQQ 60-session return, and at most three sourced candidates. Code applies the trend, momentum, relative-strength, earnings-distance, and fixed-size rules; the LLM supplies only a compact business-quality pass/fail judgment with source and as-of metadata. The policy selects at most one new position at a fixed 10% target and otherwise publishes `NO_TRADE`.

This first slice intentionally requires an empty Account A portfolio. HOLD, discretionary SELL, ROTATE, raw-bar ingestion, numeric fundamental screening, and a strategy/plugin framework are not implemented. Sourced SMA50 and return values are recorded but not recomputed from raw bars, and code validates how the LLM's business-quality judgment is used without proving that judgment economically true. Once the first position exists, the Decision lane fails closed until a later slice defines honest target weights and HOLD/SELL behavior. Execution-time deterministic Risk Exits remain independent.
