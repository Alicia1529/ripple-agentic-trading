...
- Robinhood fractional-share compatibility is a Live-only execution adaptation: when deterministic sizing leaves a fractional quantity, the emitted broker action is `MARKET + regular_hours` only if the current quote satisfies the immutable planned limit; otherwise it is rejected. Integer quantities remain LIMIT, while dry-run and Shadow semantics do not change. BUY cash reservation still uses the planned limit, but the market fill has no hard price cap and may slip beyond that price; the initial small-canary allocation accepts this risk.
- The designated owner grants standing authority for Scheduled Live Execution to place each exact deterministic broker action once without Robinhood review or per-order confirmation inside the existing small-canary allocation. An explicit manual invocation grants the equivalent one-cycle authority. This removes the broker preview alert layer but preserves immutable-plan binding, current account and market facts, deterministic risk, stable-ID duplicate checks, exact argument fidelity, credential-free evidence, and the stop-without-retry rule for every error or ambiguous outcome.
- Execution baseline matching is asymmetric for cash: positions must match exactly and current cash below the frozen baseline aborts, while current cash above it is recorded and may proceed. Deterministic BUY reservation remains capped at the frozen baseline, so interest, dividends, or settlement credits cannot silently increase the approved allocation or order quantity.
