...
**Why:** Holding everything else constant makes the comparison a clean model-capability comparison, not a confound of different strategy designs. Scheduled/cloud work consumes variable shared allowances, so a production lane whose missed run is unacceptable requires an explicitly enabled, budget-capped metered path.
...
**Why:** Low maintenance does not require collapsing to a single account when reconciliation, cost logging, and missed-run detection are automated rather than routine manual work.
...
**Why:** Useful recall was not demonstrated, variable spend is per returned resource, and content-retention terms conflict with immutable audit inputs. Those costs and policy seams add operational burden to an optional source with no measured incremental value.
...
**Why:** An authenticated session proves Robinhood access but does not give an independent runner an appropriate credential lifecycle. Production v1 therefore delegates credential persistence and refresh to the hosted platform instead of carrying the completed local prototype forward.
...
## D24 — Production v1 is one account with an explicit, minimal expansion seam

**Decision:** Ripple's first supported production release operates one Robinhood Agentic account and one decision lane. It must satisfy the live correctness requirements—deterministic risk authority, durable transactional state, restart-safe credentials, fail-closed unknown submissions, reconciliation, alerts, recovery, and a human-controlled live gate—rather than being a disposable prototype. The second live account, model A/B orchestration, multiple analyst lanes, and shadow pool are deferred until the single-account production path is stable. This revises the implementation priority established by D2/D2a/D2c/D4/D5 and the rollout scope in D17; it does not cancel the eventual comparison goal.

All correctness-critical records and narrow runtime boundaries carry an explicit `account_id` from the start. Production v1 constructs one account context; it does not implement account collections, cross-account coordination, generic strategy plugins, or a shared-account ledger. A later second account is added as another isolated context over the proven broker, repository, risk, and execution boundaries. Abstractions are introduced only when a second concrete use requires them.

**Why:** The previous plan let the eventual two-account experiment and shadow infrastructure dictate the critical path before one paper loop existed. That increased design and review cost without producing launch evidence. A production-quality single-account vertical slice reaches paper and live operation sooner while preserving the important safety boundary: the LLM proposes and plain deterministic code executes. Explicit account scoping prevents a later data migration or domain rewrite; anything more general before a second account exists is speculative complexity.

## D25 — Three-day MVP, one-cycle live gate, and local production v1

**Decision:** The delivery targets are a paper MVP after three development days and one-account live operation after one week. Production v1 runs on one controlled persistent Mac using SQLite for transactional truth, macOS Keychain for the versioned OAuth secret, a content-addressed local directory for credential-free audit blobs, and `launchd` as a trigger. This supersedes D10 for production v1 and defers the hosted deployment described there.

The pre-live observation gate is one complete scheduled production-path paper cycle: a Day T decision followed by a Day T+1 fake execution. This supersedes D18's eight-week pre-live operational gate for the first account. It does not replace correctness gates: deterministic risk, durable state, credential restart, duplicate-trigger protection, failure-injection tests, reconciliation, kill switch, recovery, and fail-closed unknown outcomes must all pass before a human may enable live mode. A date target never authorizes launch through a failed gate.

Eight continuous weeks now gate only a possible capital increase. After eight live weeks without an unresolved reconciliation, duplicate cycle, missed-run blind spot, or material risk defect, the owner may review after-cost performance, drawdown, and operational evidence and approve a specific increase. The system never increases capital automatically.

**Why:** The former eight-week pre-live gate delayed real operational evidence and encouraged building comparison and cloud infrastructure before a usable product existed. One end-to-end paper cycle gives minimal timing and scheduling evidence, while deterministic failure tests cover scenarios that waiting alone cannot prove. The already-verified local OAuth/Keychain path makes a single-machine release materially smaller than inventing an unproven hosted credential system. The trade-off is lower availability: the Mac is now production infrastructure and must remain powered, awake, connected, backed up, and monitored.

## D26 — Hosted execution routine and explicitly accepted small-account risks

**Decision:** Production v1 uses two fresh hosted LLM sessions on separate schedules. The Decision Routine has read-only market/broker access and no order tools. The Execution Routine reads the published plan and configuration, calls the checked-in deterministic risk scripts, and may call Robinhood review/place/cancel tools through the platform-managed MCP connection. The platform owns OAuth storage and refresh; Ripple does not implement a plain executor or custom credential lifecycle. Per-cycle plans, configuration, JSONL logs, and sanitized reports persist through a private Git repository. This supersedes D6's code-level execution authority, D10/D25's execution host and v1 release gates, D15's production-v1 storage split, D16's production-v1 unknown-submission guarantee, D22's production-v1 runner-owned OAuth design, and D24's production-v1 requirement for the deferred hardening mechanisms. D24's one-account scope and minimal account-scoping seam remain. The existing deterministic-core objects and feasibility spikes remain useful evidence but do not block this v1 path.

Production v1 deliberately accepts the possibility that the Execution Routine supplies incorrect inputs to a risk script, misreads or ignores its result, copies an incorrect order field, calls an order twice, retries after an ambiguous timeout, crashes after broker acceptance but before logging, changes or misreads configuration, follows prompt-injected content, or changes tool behavior after a model/prompt update. These are not claimed impossible or fully mitigated. The owner accepts them because v1 uses one dedicated small account with a $500–1000 validation allocation and prioritizes a three-day MVP and one-week launch.

Cheap guardrails remain: only the Execution Routine receives order tools; it does no news analysis or thesis generation; risk calculations stay in small reviewable scripts; `execution.mode` starts as `dry_run` and only the owner may change it; one scheduler runs each phase; malformed/missing inputs stop the current cycle; every intended decision is logged when the session remains healthy. These are operational controls, not exactly-once or code-level capability guarantees.

Before any capital increase or second live account, explicitly revisit whether to introduce a plain executor, transactional state, broker idempotency/reconciliation, versioned credentials, immutable audit storage, and prompt-injection isolation. Eight live weeks permit a review; they do not automatically waive this architecture review or increase capital.
...
- [YizhiSong/FriesTrader](https://github.com/YizhiSong/FriesTrader) — a real, deployed Robinhood Agentic trading agent with a similar decision/execution split and mechanical risk-rule scripts. Read, not forked. Its per-cycle stop-loss/take-profit recheck and cross-account wash-sale guard informed Ripple's rules. D26 now adopts the same broad hosted execution trade-off for the initial small account while retaining separate Decision/Execution sessions and documenting that deterministic arithmetic is not a code-level barrier around broker tool use.
