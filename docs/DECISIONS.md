...
- Decision runs Sunday–Thursday around 9:00 PM `America/New_York`; Execution runs the next weekday around 9:35 AM. Schedules never automatically backfill missed cycles. The designated owner may publish a live/shadow historical Decision from complete point-in-time inputs; it retains the normal Decision window and is labeled `backfill`. Execution may consume that immutable plan through the normal manual path without relaxing risk, account binding, Live Gate, or broker safeguards.
