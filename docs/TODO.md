...
- [ ] Bind Account B's hosted MCP connection to the intended account, configure its Decision and Execution schedules, and observe the same complete scheduled dry cycle.
- [ ] Keep both lanes isolated and credential-free; manual rehearsals do not count as scheduled acceptance.
...
- [ ] Verify each hosted connection selects its intended account and confirm live order shapes with non-writing or smallest-safe probes.
...
- [ ] Start each lane with its separately approved $500–1000 validation allocation and inspect its first live cycle; activation may be staggered.

## Eight-week capital review

- [ ] Operate each enabled lane for eight continuous weeks and record after-cost performance, drawdown, missed runs, reconnects, Git conflicts, ambiguous outcomes, and any divergence from deterministic output.
