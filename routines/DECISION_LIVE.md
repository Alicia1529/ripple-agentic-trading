...
Run the core tests, inspect artifacts for secrets, and commit only new credential-free lane state with a `Decision:` subject. The designated owner grants standing authorization to push that focused Decision commit immediately after verification; push normally without requesting per-run confirmation. This authorization covers only the new Decision artifacts and rolling handoff entry from this routine. It does not cover unrelated changes, force-push, Execution or broker work, credentials, or a retry after a rejected or ambiguous push. Finish with the account ID, strategy ID, plan ID, order count, tests, and commit. Do not perform Execution work.
